...
PKG := github.com/me-io/memcached-util
...
test-local:
	MEMCACHED_URL=${MEMCACHED_URL} go test -v ./...
...
test-docker: build-dirs
	...
	@docker container rm memcached-util-memcached -f > /dev/null 2>&1 || true
	@docker run                                                                   \
	    -ti                                                                       \
	    --rm 	     														      \
	...
	@docker run                                                                   \
	    ...
	    " ; 																	  \
		docker container rm memcached-util-memcached -f > /dev/null 2>&1
